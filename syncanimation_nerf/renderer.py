...
class NeRFRenderer(nn.Module):
    def __init__(self, opt):
        ...
        # aabb for point gen; bound still used for density hash
        ...
        if self.opt.torso or self.opt.special:
        ...
        density_grid = torch.zeros([self.cascade, self.grid_size ** 3])  # [CAS, H^3]
    ...
    def run_cuda(self, Xhub_feature, rays_o, rays_d, auds, bg_coords, poses,
                 eye=None, index=0, dt_gamma=0, bg_color=None,
                 perturb=False, force_all_rays=False, max_steps=1024, T_thresh=1e-4, **kwargs):

        if isinstance(poses, list) and isinstance(rays_o, list) and isinstance(rays_d, list):
            torso_poses = poses[1]
            torso_rays_o = rays_o[1]
            torso_rays_d = rays_d[1]
            poses = poses[0]
            rays_o = rays_o[0]
            rays_d = rays_d[0]
        ...
        # apply learned camera offset in train
        ...
        if enc_a is not None and self.smooth_lips:
            if self.enc_a is not None:
                _lambda = 0.35  # inter-frame smoothing
        ...
        if self.individual_dim > 0:
            if self.training:
                ind_code = self.individual_codes[index] # 1×4
            # fixed ind code at test
            else:
        ...
        if self.training:
            ...
            xyzs, dirs, deltas, rays = raymarching.march_rays_train(rays_o, rays_d,
                                                                    self.bound, self.density_bitfield,
        ...
        if self.opt.special and False:
            torso_results = self.run_torso(Xhub_feature, torso_rays_o, bg_coords, torso_poses, index, bg_color, **kwargs)
        else:
            torso_results = self.run_torso(Xhub_feature, rays_o, bg_coords, poses, index, bg_color, **kwargs)
        bg_color = torso_results['bg_color']


        image = image + (1 - weights_sum).unsqueeze(-1) * bg_color #image + (1 - weights_sum).unsqueeze(-1) * bg_color
    ...
    def run_torso(self, Xhub_feature, rays_o, bg_coords, poses, index=0, bg_color=None, **kwargs):
        is_testing = kwargs["test"]
        ...
        if self.torso or (self.opt.special and is_testing) or self.opt.finetune_lips:  # disable during face-only train
            if self.opt.special:
                with torch.no_grad():
                    if self.individual_dim_torso > 0:
                        if self.training:
                ...
                if mask.any():
                    torso_alpha_mask, torso_color_mask, deform = self.forward_torso(bg_coords[mask], poses,
                                                                                    ind_code_torso)
            ...
            if self.opt.test:  # requires torso=True for correct bg
        ...
        for xs in X:
            for ys in Y:
                for zs in Z:
                    ...
                    indices = raymarching.morton3D(coords).long()  # [N] Morton code
                    ...
                    for cas in range(self.cascade):
                        ...
                        while head < B:
                            ...
                            # world-to-camera (c2w poses)
                            cam_xyzs = cas_world_xyzs - poses[head:tail, :3, 3].unsqueeze(1)  # world to camera
    ...
    def update_extra_state(self, decay=0.95, S=128):
        ...
        rand_idx = random.randint(0, self.aud_features.shape[0] - 1)    # aud_features shape is [5520, 1, 512]
    ...
    def render(self, Xhub_feature, rays_o, rays_d, auds, bg_coords, poses, staged=False, max_ray_batch=4096, **kwargs):
        ...
        if staged and not self.cuda_ray:
            ...
            results = _run(Xhub_feature, rays_o, rays_d, auds, bg_coords, poses, **kwargs)

        return results
    
    
    def render_torso(self, Xhub_feature, rays_o, rays_d, auds, bg_coords, poses, staged=False, max_ray_batch=4096, **kwargs):
        ...
        if staged and not self.cuda_ray:
            ...
            results = _run(Xhub_feature, rays_o, bg_coords, poses, **kwargs)
