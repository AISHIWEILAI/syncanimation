...
def extract_audio_features(path, mode='hubert'):

    print(f'[INFO] ===== extract audio labels for {path} =====')
    if mode == 'hubert':
        cmd = f'python data_utils/hubert.py --wav {path}'
        os.system(cmd)
        xhu_path = path.replace('.wav', '_Xhu.npy').replace('aud.wav', 'aud_Xhu.npy')
        if not os.path.isfile(xhu_path):
            print(f'[WARN] aud_Xhu not found at {xhu_path}. '
                  f'Please refer to the official FaceXHuBERT repository for installation and inference: '
                  f'https://github.com/galib360/FaceXHuBERT')
...
# Export camera poses to transforms_train/val JSON.
...
if __name__ == '__main__':
    ...
    parser.add_argument('--asr', type=str, default='hubert', help="hubert")
